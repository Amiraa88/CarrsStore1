...
#str_punc = re.sub("[\.\(\)\[\],!\?]", "", string.punctuation)
...
def clean(text):
    ...
    text = re.sub("http.*?([ ]|\|\|\||$)", repl, text).lower()
...
with open('DomTDomF.pickle', 'rb') as file:
    thinker_feeler = pickle.load(file)
    
with open('DomNDomS.pickle', 'rb') as file:
    intuitive_sensor = pickle.load(file)
    
sentence = 'I organize patterns and find the relation between things, i can foresee future implicitly and internalize all of my thoughts through symbols and meaningful'




def process_classify(sample, model):
    clean_sample = clean(sample)
    x = model["cv"].transform([clean_sample]).toarray()
    y = model["model"].predict(x)
    probability = model["model"].predict_proba(x)
    decoded_result = model["labelEncoder"].inverse_transform(y)
    
    return decoded_result, probability


result, probability = process_classify(sentence, thinker_feeler)
print(result, probability)


result, probability = process_classify(sentence, intuitive_sensor)
print(result, probability)